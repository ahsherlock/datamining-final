...
data=np.loadtxt("dumbData.txt",delimiter=",")
budget=data[:,:1]
(m,k)=budget.shape
profit=data[:,1:2]
plotlib.scatter(budget,profit,color="red",marker="*")
plotlib.xlabel("budget in 10,000s")
plotlib.ylabel("profit in 10,000$")
plotlib.title("Video Game Profit")
#plotlib.show()

population_matrix=np.c_[np.ones([m,1]),budget]
alpha=0.01
epoch=1500
theta=np.zeros([2,1])
...
plotlib.plot(budget,predicted_profit)
plotlib.show()